...
class GenericTorrentScraper(object):
	...
	def _parse_torrent(self, row, row_tag):
		...
		if magnet_link is not None:
			...
			if 'rutor' in str(torrent.title):
				torrent.title = row.split('<a href="/')[1].split('">')[1].split('</a')[0]
				torrent.size = row.split('<td align="right">')[1].split('</td><td align="center">')[0].replace('&nbsp;',' ')
				torrent.seeds = row.split('" alt="S" />')[1].split('</span>')[0].replace('&nbsp;',' ').strip()
				return torrent
