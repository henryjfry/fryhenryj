...
def trakt_movies_latest_4k_releases(raw=False):
	results = sorted(Trakt.get_latest_4k_releases_movies(), key=lambda k: k['listed_at'], reverse=True)
	if raw:
		return results
	else:
		return list_trakt_movies(results)
...
@plugin.route('/movies/trakt/random_latest_4k_releases')
def trakt_movies_play_random_latest_4k_releases():
	results = trakt_movies_latest_4k_releases(raw=True)
	trakt_movies_play_random(results)
